...
import { Plus, Trash2, Search, Filter, Download, X, ImageIcon, Loader2, FileText, Check, AlertCircle, Copy } from 'lucide-react';
...
export function ProductManagement() {
  ...
  const handleEdit = (product: Product) => {
    ...
    setImageUrlsText((Array.isArray(product.images) ? product.images : (typeof product.images === 'string' ? [product.images] : [])).join('\n'));
    setActiveTab('basic');
    setShowForm(true);
  };
                    ...
                    <tr key={product.id} className="hover:bg-gray-50/50 transition-colors group cursor-pointer" onClick={() => handleEdit(product)}>
                      ...
                      <td className="py-4 px-8">
                         <div className="flex flex-col gap-1.5">
                            ...
                            <div className="flex flex-wrap gap-1">
                               {(Array.isArray(product.categorySlugs) ? product.categorySlugs : typeof product.categorySlugs === 'string' ? [product.categorySlugs] : []).map(slug => (
                      ...
                      <td className="py-4 px-8">
                         <div className="flex flex-col">
                            <span className="font-black text-[#1A2B4C]">Rs. {(product.salePrice ?? 0).toLocaleString()}</span>
                            {product.regularPrice > product.salePrice && (
                              <span className="text-[10px] font-bold text-gray-400 line-through">Rs. {(product.regularPrice ?? 0).toLocaleString()}</span>
                            )}
                         </div>
                      </td>
                      <td className="py-4 px-8">
              ...
              <form id="product-form" onSubmit={handleSubmit} className="p-8 overflow-y-auto flex-grow space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="col-span-2">
                        ...
                        <input 
                          ...
                          value={(Array.isArray(formData.categorySlugs) ? formData.categorySlugs : (typeof formData.categorySlugs === 'string' ? [formData.categorySlugs] : [])).join(', ')}
                          onChange={e => setFormData({...formData, categorySlugs: e.target.value.split(',').map(s => s.trim())})}
                          className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 focus:outline-none focus:border-[#00B4D8] focus:ring-1 focus:ring-[#00B4D8] font-bold text-[#1A2B4C] transition-all text-sm" 
                        />
                      </div>

                      {/* Active & Featured Configuration */}
                      <div className="col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                        <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex items-center gap-3">
                          <input 
                            ...
                            className="w-5 h-5 rounded-lg border-gray-300 text-[#00B4D8] focus:ring-[#00B4D8] cursor-pointer"
                          />
                          <div>
                            <label htmlFor="isActive" className="text-[10px] font-black text-[#1A2B4C] uppercase tracking-widest block cursor-pointer select-none">
                              Active Status
                            </label>
                            <span className="text-[9px] text-gray-400 font-bold block mt-0.5">Show this product in public catalog navigation</span>
                          </div>
                        </div>
                        ...
                        <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex items-center gap-3">
                          ...
                          />
                          <div>
                            <label htmlFor="isFeatured" className="text-[10px] font-black text-[#1A2B4C] uppercase tracking-widest block cursor-pointer select-none">
                              Featured Showcase
                            </label>
                            <span className="text-[9px] text-gray-400 font-bold block mt-0.5">Display on the homepage featured section</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {activeTab === 'details' && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    <div>
                      <div className="flex justify-between items-center mb-2 ml-1">
                         <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Short Summary (Preview)</label>
                         <button type="button" onClick={(e) => { e.preventDefault(); navigator.clipboard.writeText(formData.shortDescription); alert('Copied'); }} className="flex items-center gap-1 text-[9px] font-bold text-[#00B4D8] uppercase tracking-widest hover:underline"><Copy size={12} /> Copy</button>
                      </div>
                      <RichTextEditor
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2 ml-1">
                         <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Full Technical Description</label>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2 ml-1">
                         ...
                         <div className="flex items-center gap-4">
                           ...
                           <button type="button" onClick={(e) => { e.preventDefault(); navigator.clipboard.writeText(formData.specifications); alert('Copied'); }} className="flex items-center gap-1 text-[9px] font-bold text-[#00B4D8] uppercase tracking-widest hover:underline"><Copy size={12} /> Copy</button>
                         </div>
                      </div>
                      <textarea 
                        rows={4}
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    <div>
                      ...
                      <div className="text-right mt-1">
                         <span className={`text-[9px] font-bold uppercase tracking-widest ${formData.seoMetaDescription.length > 150 ? 'text-rose-500' : 'text-gray-400'}`}>
                    ...
                    <div>
                      ...
                      <textarea 
                        rows={2}
                        placeholder="e.g. #videoconferencing #poly #audiovisual (comma or space separated)"
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2 ml-1">
                        ...
                        <button 
                          ...
                          onClick={(e) => {
                             e.preventDefault();
                             navigator.clipboard.writeText(formData.imageAltText);
                             alert('Copied to clipboard');
                          }}
                          className="flex items-center gap-1 text-[9px] font-bold text-[#00B4D8] uppercase tracking-widest hover:underline"
                        >
                      ...
                      <input 
                        type="text"
                        placeholder="e.g. Poly Studio X50 Video Bar mounted on display"
